...
public class DAO {
	...
	public boolean inserirSerie(Serie serie) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO series (id, qtd_episodios, qtd_temporadas, nome) "
					       + "VALUES ("+serie.getId()+ ", '" + serie.getQtdEpisodios() + "', '"  
					       + serie.getQtdTemporadas() + "', '" + serie.getNome() + "');");
			st.close();
			status = true;
			...
			ResultSet rs = st.executeQuery("SELECT * FROM series");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                series[i] = new Serie(rs.getInt("id"), rs.getInt("qtd_episodios"), 
	                		                  rs.getInt("qtd_temporadas"), rs.getString("nome"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
